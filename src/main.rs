...
use uv::scripts::{init_scripts, Script, ScriptFile};
use uv::{detail, funny_opening, output, warning};
...
use uv::address::parse_addresses;
...
fn main() {
    ...
    let mut uv_bench = NamedTimer::start("uv");
    ...
    let open_sockets = block_on(scanner.run());
    portscan_bench.end();
    benchmarks.push(portscan_bench);

    // Banner-grab / service-detect open ports via uv-engine
    if !opts.greppable {
        let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
        let enriched = rt.block_on(probe::enrich(&open_sockets, opts.timeout, opts.no_banner));
        probe::print_results(&enriched, opts.greppable);
    }
    ...
    for ip in ips {
        ...
        \n Alternatively, increase the timeout if your ping is high. uv -t 2000 for 2000 milliseconds (2s) timeout.\n",
        ip,
        opts.batch_size,
        "'uv -b <batch_size> -a <ip address>'");
    ...
    // To use the runtime benchmark, run the process as: RUST_LOG=info ./uv
    script_bench.end();
    benchmarks.push(script_bench);
    uv_bench.end();
    benchmarks.push(uv_bench);
    debug!("Benchmarks raw {benchmarks:?}");
    info!("{}", benchmarks.summary());
}

/// Prints the opening title of uv
...
fn infer_batch_size(opts: &Opts, ulimit: usize) -> usize {
    ...
    if ulimit < batch_size {
        ...
        if ulimit < AVERAGE_BATCH_SIZE {
            ...
            warning!("Your file limit is very small, which negatively impacts uv's speed. Up the Ulimit with '--ulimit 5000'. ", opts.greppable, opts.accessible);
