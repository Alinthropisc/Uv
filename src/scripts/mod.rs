...
//! ### `default`
//!
//! This is the default behavior, like as it was from the beginning of uv.
//!
//! The user do not have to chose anything for this. This is the only script
//! embedded in uv running as default.
//!
//! ### `none`
//!
...
//!
//! uv will look for the script configuration file in the user's home
//! dir: `home_dir/.uv_scripts.toml`
//!
//! The config file have 3 optional fields: `tag`, `developer` and `port`. Just
//! the `tag` field will be used forther in the process.
//!
//! uv will also look for available scripts in the user's home dir:
//! `home_dir/.uv_scripts` and will try to read all the files, and parse
//! them into a vector of [`ScriptFile`].
//!
//! Filtering on tags means the tags found in the `uv_scripts.toml` file
//! will also have to be present in the [`ScriptFile`], otherwise the script
//! will not be selected.
//!
//! All of the `uv_script.toml` tags have to be present at minimum in a
//! [`ScriptFile`] to get selected, but can be also more.
//!
//! Config file example:
//!
//! - `fixtures/test_uv_scripts.toml`
//!
...
static DEFAULT: &str = r#"tags = ["core_approved", "uv", "default"]
developer = [ "uv", "https://github.com/uv" ]
ports_separator = ","
...
mod tests {
    ...
    fn find_and_parse_scripts() {
        let scripts = find_scripts("fixtures/.uv_scripts".into()).unwrap();
    ...
    fn open_script_file_invalid_headers() {
        ScriptFile::new("fixtures/.uv_scripts/test_script_invalid_headers.txt".into()).unwrap();
    }

    #[test]
    #[should_panic]
    fn open_script_file_invalid_call_format() {
        let mut script_f = ScriptFile::new("fixtures/.uv_scripts/test_script.txt".into()).unwrap();
    ...
    #[test]
    fn parse_txt_script() {
        let script_f = ScriptFile::new("fixtures/.uv_scripts/test_script.txt".into()).unwrap();
        assert_eq!(
            script_f.tags,
    ...
    #[cfg(unix)]
    fn run_bash_script() {
        let script_f = ScriptFile::new("fixtures/.uv_scripts/test_script.sh".into()).unwrap();
        let script: Script = into_script(script_f);
    ...
    fn run_python_script() {
        let script_f = ScriptFile::new("fixtures/.uv_scripts/test_script.py".into()).unwrap();
        let script: Script = into_script(script_f);
        let output = script.run().unwrap();
        // output has a newline at the end by default, .trim() trims it
        assert_eq!(
            output.trim(),
            "Python script ran with arguments ['fixtures/.uv_scripts/test_script.py', '127.0.0.1', '80,8080']"
        );
    }
    ...
    #[cfg(unix)]
    fn run_perl_script() {
        let script_f = ScriptFile::new("fixtures/.uv_scripts/test_script.pl".into()).unwrap();
        let script: Script = into_script(script_f);
        let output = script.run().unwrap();
        // output has a newline at the end by default, .trim() trims it
        assert_eq!(output.trim(), "Total args passed to fixtures/.uv_scripts/test_script.pl : 2\nArg # 1 : 127.0.0.1\nArg # 2 : 80,8080");
    }
    ...
    fn test_custom_directory_config() {
        ...
        let config_str = r#"
            ...
            directory = "fixtures/.uv_scripts"
        "#;

        let config: ScriptConfig = toml::from_str(config_str).unwrap();
        assert_eq!(config.directory, Some("fixtures/.uv_scripts".to_string()));
