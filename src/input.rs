...
///   - default will run the default embedded nmap script.
...
/// uv — Ultra-fast async port scanner. masscan speed × nmap intelligence. Rust + C23.
/// WARNING: Do not use against sensitive infrastructure without authorization.
/// Rapid socket flood can overwhelm targets.
pub struct Opts {
    ...
    /// If set to 0, uv will correct it to 1.
    ...
    /// To use the argument -A, end uv's args with '-- -A'.
    /// Example: 'uv -t 1500 -a 127.0.0.1 -- -A -sC'.
...
mod tests {
    ...
    #[parameterized(input = {
        vec!["uv", "--addresses", "127.0.0.1"],
        vec!["uv", "--addresses", "127.0.0.1", "--", "-sCV"],
        vec!["uv", "--addresses", "127.0.0.1", "--", "-A"],
        vec!["uv", "-t", "1500", "-a", "127.0.0.1", "--", "-A", "-sC"],
        vec!["uv", "--addresses", "127.0.0.1", "--", "--script", r#""'(safe and vuln)'""#],
    }, command = {
        vec![],
        vec!["-sCV".to_owned()],
        vec!["-A".to_owned()],
