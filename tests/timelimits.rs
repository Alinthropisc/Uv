/*
 * Test uv against different targets with a time limit.
 * The tests assumes target/debug/uv has already been built.
 *
...
fn run_uv_with_timeout(args: &[&str], timeout: Duration) {
    println!("Running: target/debug/uv: {}", args.join(" "));

    use std::time::Instant;

    let start = Instant::now();

    let mut child = Command::new("target/debug/uv").args(args).spawn().unwrap();
...
mod timelimits {
    ...
    fn scan_localhost() {
        ...
        super::run_uv_with_timeout(&["--greppable", "--no-nmap", "127.0.0.1"], timeout);
    }

    #[test]
    #[ignore]
    fn scan_google_com() {
        super::run_uv_with_timeout(
            &[
                "--greppable",
                "--no-nmap",
                "-u",
    ...
    fn udp_scan_localhost() {
        let timeout = super::Duration::from_secs(25);
        super::run_uv_with_timeout(&["--greppable", "127.0.0.1", "--udp"], timeout);
    }
    #[test]
    #[ignore]
